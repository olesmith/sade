...
class ItemsLatex extends Item
{
    ...
    function LatexItems($items=array())
    {
        ...
        $glue=$this->GetLatexSkel($latexinfo[ "Glue" ]);
        $head="";//$this->GetLatexSkel($latexinfo[ "Head" ]);
        $tail="";//$this->GetLatexSkel($latexinfo[ "Tail" ]);
    ...
    {
        if  (count($items)==0) { $items=$this->ItemHashes; }
        $this->LogMessage("PrintItems",count($items)." items");
    ...
    function ItemLatexTablesPrint($items=array())
    {
        ...
        if (count($items)==0) { $items=$this->ItemHashes; }
        $this->LogMessage("ItemLatexTablesPrint",count($items)." items");
        $latex="";
        foreach ($items as $id => $item)
        {
