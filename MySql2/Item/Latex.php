...
class ItemLatex extends ItemPostProcess
{
    ...
    function TrimLatexItem($item=array())
    {
        ...
        foreach ($item as $key => $value)
        {
            ...
            {
                if ($this->ItemData[ $key ][ "LatexCode" ])
                {
                    $value=preg_replace('/&/',"\\&",$value);
                    $value=preg_replace('/\^/',"\\^",$value);
                }

                if ($this->ItemData[ $key ][ "Sql" ]=="FILE" && !is_array($value))
                {
                    $value=basename($value);
                    $value=preg_replace('/^\./',"",$value);
                }
                if ($this->ItemData[ $key ][ "TimeType" ]==1)
                {
                    $value=$this->TimeStamp2Text($value);
                }

                if ($this->ItemData[ $key ][ "IsHour" ])
                {
                    $value=$this->CreateHourShowField($key,$item,$value);
                }
                if ($this->ItemData[ $key ][ "LatexFormat" ])
                {
    ...
    {
        $this->LogMessage("ItemLatexTablePrint",$item[ "ID" ].": ".$this->GetItemName($item));
