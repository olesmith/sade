...
class ItemEdits extends ItemForms
{
    ...
    {
        if (count($item)==0) { $item=$this->ReadPostItem(); }
        ...
        if ($this->ItemIsUnique($item))
        {
            ...
            {
                if (!isset($item[ $data ]) || $item[ $data ]=="") { $item[ $data ]=$value; }
            }
            ...
            //$this->LogMessage("Add",$this->GetItemName($item));
            ...
            {
                if (!isset($this->ItemData[ $data ]) || $this->ItemData[ $data ][ "Derived" ])
                {
                    unset($ritem[ $data ]);
                }
            }
    ...
    function Copy($item=array())
    {
        ...
        if ($this->ItemIsUnique($item))
        {
            //$this->LogMessage("Copy",$item[ "ID" ].": ".$this->GetItemName($item));
    ...
    function Delete($item=array(),$echo=TRUE)
    {
        ...
        $this->LogMessage("Delete",$item[ "ID" ].": ".$this->GetItemName($item));
        $this->MySqlDeleteItem($this->SqlTableName(),$item[ "ID" ]);
