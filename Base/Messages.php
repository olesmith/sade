...
class SqlMessages extends Cookies
{
    ...
    function PrintCallStack($parm=FALSE)
    {
        $debug=debug_backtrace($parm);
        for ($n=0;$n<count($debug);$n++)
        {
            if (!empty($debug[$n][ 'object' ]))
            {
                $debug[$n][ 'object' ]=get_class($debug[$n][ 'object' ]);
            }
            else
            {
                $debug[$n][ 'object' ]="-";
        ...
        array_unshift($table,$this->B($titles));
        array_unshift($table,$this->H(3,"CallStack"));
